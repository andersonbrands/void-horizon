...
//***********************************************************************
//a more powerful assert that can include messages
...
#define MPOD_VERIFY_FAIL(msg) \
	do { if (Assert::report(nullptr, (msg), __FILE__, __LINE__)) \
	MPOD_HALT(); } while(0)
...
//#define REMOVE_ASSERTS
	...
	#define MPOD_ASSERT_FAIL(msg) do { (void)sizeof(msg); } while(0)
		...
		do { if (Assert::report(nullptr, (msg), __FILE__, __LINE__)) (void)sizeof(cond); } while(0)
	#else
		...
		#define MPOD_ASSERT_FAIL(msg) \
		do { if (MPOD::Assert::report(nullptr, (msg), __FILE__, __LINE__)) \
		MPOD_HALT(); } while(0)
	#endif
#endif
